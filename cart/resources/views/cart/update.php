...
if (isset($_SESSION['Account_ID']) == ' ' && ($_SESSION['Account_Description']) == 'Admin'){
    ...
    $query = mysqli_query($dbconnect, "SELECT * FROM products WHERE id='".$_GET['id']."'");
...
?>
    ...
    <span class="icon-bar"></span>
        ...
        <div class="container">
            <div align="center">
                <h2 class="soledph"><strong>UPDATE PRODUCT</strong></h2>
                <hr class="hr-short">
            </div><br><br><br><br>
            <form action="" method="POST">
                <input type="hidden" value="<?=($_GET['id'])?$_GET['id']:''?>" style="border-radius:5px;">
                <div class="row">
                    <div class="col-md-6">
                        <img src="<?=$row['image']?>" height="300px;">
                    </div>
                    <div class="col-md-5">
                        <h3 class="white-text">Product Name: <span class="soledph"><?=$row['title']?></span></h3>
                        <h3 class="white-text">Description: <span class="soledph"><?=$row['description']?></span></h3>
